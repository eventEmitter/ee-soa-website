!function(){
	...
	module.exports = new Class({
		...
		, init: function(path) {
			...
			this.nunjucksContext = new nunjucks.Environment(new nunjucks.FileSystemLoader(this.templateRootPath), this.config.nunjucks);
		...
		, registerExtension: function(name, extension) {
			this.nunjucksContext.addExtension(name, extension);
		}
		...
		, _loadWebFiles: function() {
			if (this.config.wwwFiles) {
				...
				this.wwwFiles.load(this.wwwFilesPath, function(err){
					this._loaded('middleware', this.middlewares, err);
				}.bind(this));
			}
			else this._loaded('middleware', this.middlewares);
		}
		 ...
		 */
		, _loaded: function(what, value, err) {
			this._status[what].loaded = true;
			this._status[what].value = value;
			this._status[what].callbacks.forEach(function(cb){cb(err, value);});
			this.emit(what+'Load');
