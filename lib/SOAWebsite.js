!function(){
	...
	module.exports = new Class({
		...
		, init: function(path) {
			...
			this.waiter.on('load', function(err){ this.emit('load', err); }.bind(this));
			...
			if (!this.configfile.environment) throw new Error('Please start the application using one of the following flags: live, staging, dev, testing. Never execute the live environment if you are not running on the live environment!');
			...
			this.nunjucksEnvironment = new nunjucks.Environment(new nunjucks.FileSystemLoader(this.templateRootPath), this.config.nunjucks);
